...
def predict(model_path, test_path, output_path, predictionType="classification", outputType="int",
    ...
    #print(model.get_input_space().make_theano_batch().axes)
    Y = model.fprop(X)
    ...
    z = np.asarray( x.reshape(x.shape[0],28,28,1) , dtype=theano.config.floatX)
   ...
   #     y = []
    #
  #  for i in range(28):
  #      xtmp = x[i*1000:(i+1)*1000 , : ]
  #      z = np.asarray( xtmp.reshape(xtmp.shape[0],28,28,1) , dtype=theano.config.floatX)
  #      tmpy = f(z)
  #      for yy in tmpy:
 #           y.append(yy)
#        print(i)
#
    ...
    y = np.asarray(y)
    qq= []
    qq.append(['ImageId' , 'Label'])
    for i in range(y.shape[0]):
        qq.append([i+1, y[i]])

    with open('outpy.csv' , 'wb') as f:
            writer = csv.writer(f)
            writer.writerows(qq)
